...
//USING POISONS EQUATION AS SOURCE ARE TAKEN INTO ACCOUNT AND GaUSS SEIDEL ITERATION METHOD IS ADOPTED
...
//(T) IS VECTOR OF UNKNOWN TEMPERATURE AT EACH NODE IN THE FRAME EXCLUDING BOUNDRIES
    ...
    double maxdiff=1;
//////////////////////////////Jacobi method///////////////////////////////////////
    while(maxdiff > 0.01){//IF diff bw previous and current valof temp >0.01 enter the loop we want that diff to be very small
      maxdiff=0; //INITIALISING 
        for(int i=0;i<sms;i++){
            double sum=0;
            for(int j=0;j<sms;j++){
                if(i!=j){
                    sum=sum+(coeff[i*sms+j]*T[j]);//Gauss seidel
                }
            }
            double new_val=((bmat[i]-sum)/coeff[i*sms+i]);
            maxdiff=fmax(maxdiff,fabs(new_val-T[i]));
            T[i]=new_val;
        }
        count+=1;
    }
